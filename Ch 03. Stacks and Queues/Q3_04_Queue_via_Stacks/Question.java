...
public class Question {
	public static void main(String[] args) {
		...
		MyCodeQ304<Integer> MyQueue = new MyCodeQ304<>();
		for (int i = 0; i < 100; i++) {
			...
			if (choice <= 5) { // enqueue
				...
				int top3 = MyQueue.remove();
				if (top1 != top2|| top3!=top2) { // Check for error
					System.out.println("******* FAILURE - DIFFERENT TOPS: " + top1 + ", " + top2 + ", "+ top3);
				} 
				System.out.println("Dequeued " + top1);
			}
			
			if (test_queue.size() == my_queue.size()&&MyQueue.size()==my_queue.size()) {
				if (test_queue.size() > 0 && (test_queue.peek() != my_queue.peek()||MyQueue.peek()!=my_queue.peek())) {
					System.out.println("******* FAILURE - DIFFERENT TOPS: " + test_queue.peek() + ", " + my_queue.peek() +" , "+MyQueue.peek()+ " ******");
				}
			} else {
