...
public class Tester {
	...
	public static void main(String[] args) {
		int max = 1000;
		int counterA = 0, counterB = 0, counterC = 0, counterD = 0,myCounter=0;
		...
		for (int a = 0; a < max; a++) {
			for (int b = 0; b < max; b++) {
				...
				int myprod = MyCodeQ805.minProduct(a,b);
				...
				myCounter+= MyCodeQ805.counter;

				QuestionA.counter = 0;
				QuestionB.counter = 0;
				QuestionC.counter = 0;
				QuestionD.counter = 0;
				MyCodeQ805.counter=0;
				
				if (prodA != product || prodB != product || prodC != product || prodD != product ||myprod!=product) {
					System.out.println("Failure: " + a + " * " + b + " = " + product + " instead of (" + prodA + ", " + prodB + ", " + prodC + ", " + prodD +", "+myprod+ ")");
					break;
				} 
			}
		}
